...
class KDNetwork(nn.Module):
  ...
  def __init__(self, C, num_classes, layers, criterion_kd,steps=4, multiplier=4, stem_multiplier=3,
               parse_method='darts', op_threshold=None):
    super(KDNetwork, self).__init__()
    self._C = C
    self._num_classes = num_classes
    self._layers = layers
    
    self._criterion_kd = criterion_kd
    self._steps = steps
    self._multiplier = multiplier
  ...
  def new(self):
    model_new = Network(self._C, self._num_classes, self._layers, self._criterion_kd).cuda()
  ...
  def _loss_kd(self, input, target,input2,teachermodel): 
    # print("shape1:{}".format(input.shape))
    studentout = self(input)
    # print("shape1:{}".format(studentout.shape))
    teacherout = teachermodel(input)
    teacherout = teacherout.detach()  # 切断老师网络的反向传播  
    # print(logits.shape)
    # print("=---------")
    # print(teacherout.shape)
    return self._criterion_kd(studentout, target, input2,teacherout)
