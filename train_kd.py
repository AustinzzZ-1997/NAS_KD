...
from model import NetworkImageNet as VegeNetwork
...
class TrainNetwork(object):
    ...
    def _init_log(self):
        self.args.save =  './logs/train_kd/' + '{}-{}'.format(args.teacher_arch,args.dataset) + '/{}-{}'.format(args.save, time.strftime('%Y%m%d-%H%M%S'))
        dutils.create_exp_dir(self.args.save)
    ...
    def _init_teacher(self):
      self.teacher_model = dutils.get_network(args)
      teacher_weight = dutils.get_teacher_weight(args)
      #   kd_save_path = './logs/train-resnet50_cifar100-20211106-1904/weights.pt'
      self.teacher_model.load_state_dict(torch.load(
          teacher_weight))  # 仅加载参数
      self.teacher_model.cuda()

    def _KD_loss(self,outputs, labels, teacher_outputs):
        alpha = self.args.alpha  # 0.8
        T = self.args.T
    ...
    def _init_data_queue(self):
        ...
        self.train_queue,self.valid_queue  = dutils.get_dataloader(args)

    def _init_model(self):
        genotype = eval('genotypes.%s' % self.args.arch)
        model = Network(self.args.init_channels, self.args.classes, self.args.layers, self.args.auxiliary, genotype, self.args.parse_method)
    ...
    def run(self):
        ...
        for epoch in range(self.args.start_epoch, self.args.epochs):
            self.scheduler.step()
            self.logger.info('epoch % d / %d  lr %e', epoch, self.args.epochs, self.scheduler.get_last_lr()[0])
            ...
            if valid_acc_top1 > self.best_acc_top1:
                ...
                is_best = True
                dutils.save(self.model, os.path.join(args.save, 'weights.pt'))
    ...
    def train(self):
        ...
        for step, (input, target) in enumerate(self.train_queue):
            ...
            loss = self._KD_loss(logits, target,teacher_outputs)
